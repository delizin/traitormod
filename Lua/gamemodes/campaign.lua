...
function gm:Start()
    ...
    Hook.Add("characterDeath", "Traitormod.Campaign.CharacterDeath", function(character, affliction)
    ...
    for key, value in pairs(Client.ClientList) do
        ...
        then
            ...
            if role ~= nil then
                ...
                table.insert(newRoles, Traitormod.RoleManager.Roles['Crew']:new())
            end
        end
    end

    Traitormod.RoleManager.AssignRoles(characters, newRoles)
end
...
function gm:End()
    ...
    Hook.Remove("characterDeath", "Traitormod.Campaign.CharacterDeath");
end
...
function gm:SelectAntagonists(role)
    ...
    Timer.Wait(function()
        ...
        -- In Campaign Mode we roll for the amount of traitors every round with amountTraitors acting as our maximum
        local maxAmountTraitors = this.AmountTraitors(playerInGame)
        local traitorChance = Traitormod.Config.TraitorChance
        local amountTraitors = 0
        
        -- Roll to see if we have traitors this round
        if math.random(1, 100) <= traitorChance then
            -- If we have traitors, roll for the number of traitors
            amountTraitors = math.random(1, maxAmountTraitors)
        end

        -- If DebugMode is enabled set amountTraitors to 1
...
function gm:Think()
    ...
    if not self.Ending and Game.RoundStarted and self.EndOnComplete and ended then
        ...
        Traitormod.Log("Campaign gamemode complete. Ending round in " .. delay)
